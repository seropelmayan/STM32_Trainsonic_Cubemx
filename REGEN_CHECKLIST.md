...
| Define | Set to |
|---|---|
| `PID_SPEED_KP_DEFAULT` | `312/(SPEED_UNIT/10)` (= old 10000 at KPDIV 512; manual-Kp fallback only) |
| `PID_SPEED_KI_DEFAULT` | `10000/(SPEED_UNIT/10)` |
| `SP_KPDIV` / `SP_KPDIV_LOG` | `16` / `LOG2((16))` (speed-window Kp range; 512 overflows int16 at heavy weights) |
| `MAX_APPLICATION_SPEED_RPM` | `1200` (bounds the speed REFERENCE only; CHECK_BOUNDARY rejects ramps above it) |
| `OV_VOLTAGE_THRESHOLD_V` / `UD_VOLTAGE_THRESHOLD_V` | `70.0` / `42.0` (15S pack) |
| `IQMAX_A` / `NOMINAL_CURRENT_A` | `29.0` |
...
hard ceiling `g_spdcap_hard_rpm`); the legacy `g_spdcap_*` governor is kept behind
CDC `#`. Re-apply after regen.
...
- `USER CODE` / `USER SECTION` blocks: `FOC_CalcCurrRef` (speed-source override +
  step-inject + legacy governor), `Ropetow_*` functions incl. `Ropetow_SpeedWindowApply`
  and the FW **shadow PID** in `Ropetow_McFwInit` (FW must NOT be bound to
  `PIDSpeedHandle_M1`: it rewrites that PID's integral limits every tick and would undo
  the speed-window torque limits), `Ropetow_EncoderUpdate`, CDC commands, `main.c` boot
  defines (`ENC_ONLY`, `STEP_MODE`) and `Ropetow_EspLinkService`, `mc_app_hooks.c`.
