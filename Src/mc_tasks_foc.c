...
volatile float    g_spdcap_hard_rpm  = 900.0f; /* ABSOLUTE firmware ceiling: the governor always enforces
                                                  min(requested, hard), and enforces hard even when the
                                                  request is 0/absent. NOT writable from the ESP link --
                                                  the STM32 has the last word on top speed.
                                                  RAISED 550 -> 900 (2026-09-08) so the governor BRAKE is a
                                                  true backstop rather than the normal rewind limiter. Top
                                                  speed is now set by REWIND_* (feel) and MODCAP_* (physics)
                                                  below, which fade the COMMAND instead of braking against
                                                  it. 900 sits above the highest natural voltage-limited
                                                  speed (~844 rpm at a 59 V pack, Ke 49.4 V/kRPM, linear
                                                  SVPWM) and below the 1150 rpm over-speed fault in
                                                  mc_config_common.c. Only affects MOTOR-driven motion --
                                                  the brake and roll-off are both gated on q*spd > 0, so
                                                  pulls are untouched. REVERT TO 550 to restore the old
                                                  braked-rewind behaviour. */
...
/* ---- Rewind shaping (Ropetow): two stages that FADE THE COMMAND, never brake ---
   Replaces "accelerate the handle home at the full weight setting until the 550 rpm
   governor brakes it". Braking against your own torque command burns current to
   fight yourself (see REWIND_DERATE_ESP_PATCH.md); simply not commanding the force
   is smooth and free. Both stages only ever REDUCE |q|, and both are gated on
   MOTOR-DRIVEN motion (q and speed same sign), so pull purity is untouched.

   Stage 1 REWIND_* -- feel. Fades retract torque from the full weight setting down
   to a keep-tension floor as inbound speed rises, so a dumped handle spools in
   briskly but gently, and blends the weight back in if the user catches the flying
   cable. MEMORYLESS by design: a pure function of present velocity, no state, no
   LPF, no binary gate. That rule is inherited from the governor bench war -- every
   filtered/latched term in this path leaked across direction changes and was felt
   as a tension dropout at the pull->release transition.

   Stage 2 MODCAP_* -- physics ceiling. Fades the command as the inverter runs out
...
#define REWIND_FADE_LO_RPM   150.0f  /* below this: full commanded weight            */
#define REWIND_FADE_HI_RPM   400.0f  /* at/above this: the keep-tension floor        */
...
volatile float    g_rewind_floor_a = 2.0f;   /* keep-tension floor, A (~7 kg). 0 = stage 1 off */
volatile float    g_modcap_factor  = 1.0f;   /* telemetry: modulation ceiling factor, 1 = idle  */
volatile float    g_rewind_factor  = 1.0f;   /* telemetry: velocity derate factor,   1 = idle   */
...
__weak void FOC_CalcCurrRef(uint8_t bMotor)
{
     ...
     step-test. g_spdcap_brake_now_a publishes the applied brake for telemetry. */
  {
    ...
    if ((bMotor == M1) && (g_inj_override == 0U) && (Mci[M1].State == RUN))
    {
      ...
      /* The brake exists ONLY for motor-driven overspeed (q and speed same
         sign). During HUMAN-driven motion (pulling) it may not apply -- and it
         must not ACCUMULATE either. Earlier code kept winding the integral+LPF
         through blocked fast pulls (up to ~14 A of stale state), which then
         leaked onto the rewind the instant q*spd flipped sign: a 0.3-3 A
         residue hit the stronger-of clamp and REPLACED several amps of rewind
         command for a few ticks (bench log: Iqref +318 at spd=-498 right after
         a released 846 rpm pull) -- felt as a violent tension dropout at every
         pull->release transition, independent of FW/cogging. Gate the STATE:
         while not motor-driven, the brake is fully discharged. */
      if (((int32_t)q * spd_rpm) > 0)                  /* motor-driven motion */
      {
  ...
  /* ---- Rewind shaping, stage 1 + 2 (see the REWIND_ and MODCAP_ defines) ------
     Cascade of pure |q| reductions: velocity derate (feel) -> modulation ceiling
     (physics) -> low-pack taper (battery) -> the governor brake remains only as a
     backstop above g_spdcap_hard_rpm. */
  {
    ...
    if ((bMotor == M1) && (g_inj_override == 0U) && (Mci[M1].State == RUN))
    {
      ...
      /* -- stage 1: velocity derate (memoryless; NO filter, NO latch) -- */
      if ((driven != 0U) && (g_rewind_floor_a > 0.0f))
      {
        float fade = ((float)aspd - REWIND_FADE_LO_RPM)
                     / (REWIND_FADE_HI_RPM - REWIND_FADE_LO_RPM);
        if (fade < 0.0f) { fade = 0.0f; }
        if (fade > 1.0f) { fade = 1.0f; }
        g_rewind_factor = 1.0f - fade;
        if (fade > 0.0f)
        {
          /* Blend the command toward a signed keep-tension floor. The floor never
             RAISES the command: at a weight setting lighter than the floor the
             commanded value already wins. */
          int32_t fl = (int32_t)(g_rewind_floor_a * (float)CURRENT_CONV_FACTOR);
          if (q < 0) { fl = -fl; }
          if (((q >= 0) && (fl > (int32_t)q)) || ((q < 0) && (fl < (int32_t)q))) { fl = (int32_t)q; }
          qnew = (int16_t)((float)q + (((float)fl - (float)q) * fade));
        }
      }
      else { g_rewind_factor = 1.0f; }

      /* -- stage 2: modulation ceiling -- */
      {
        float f = 1.0f;
        if (driven != 0U)
        {
